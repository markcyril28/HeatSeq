...
#===============================================================================
# Processes RSEM quantification data through DESeq2 and generates heatmaps
...
source "modules_method_5_bowtie2/0_configurations.sh"
source "modules_method_5_bowtie2/1_logging.sh"
...
if [ "$RUN_MATRIX_CREATION" = true ]; then
    ...
    Rscript --no-save modules_method_5_bowtie2/a_tximport_rsem_to_matrices_m5.R
elif [ $? -ne 0 ]; then
...
if [ "$RUN_BASIC_HEATMAP" = true ]; then
    ...
    log_info "Running modules_method_5_bowtie2/3_make_heatmap_of_matrices.R"

    Rscript --no-save modules_method_5_bowtie2/3_make_heatmap_of_matrices.R
...
if [ "$RUN_CV_HEATMAP" = true ]; then
    ...
    log_info "Processing gene groups: ${GENE_GROUPS[*]}"
    log_info "Overwrite existing files: $OVERWRITE_EXISTING"
    log_info "Running modules_method_5_bowtie2/4_make_heatmap_with_CV_of_matrices.R"

    Rscript --no-save modules_method_5_bowtie2/4_make_heatmap_with_CV_of_matrices.R
...
if [ "$RUN_BAR_GRAPHS" = true ]; then
    ...
    log_info "Processing gene groups: ${GENE_GROUPS[*]}"
    log_info "Overwrite existing files: $OVERWRITE_EXISTING"
    log_info "Running modules_method_5_bowtie2/5_make_BarGraph_of_matrices.R"

    Rscript --no-save modules_method_5_bowtie2/5_make_BarGraph_of_matrices.R
