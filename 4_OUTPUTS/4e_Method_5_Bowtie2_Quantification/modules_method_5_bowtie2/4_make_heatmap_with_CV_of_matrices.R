...
CV_HEATMAP_OUT_DIR <- "ALL_HEATMAPS_CONSOLIDATED/CV_Heatmaps"
...
dir.create(CV_HEATMAP_OUT_DIR, recursive = TRUE, showWarnings = FALSE)
dir.create(CONSOLIDATED_OUT_DIR, recursive = TRUE, showWarnings = FALSE)
...
cat("CV HEATMAP GENERATION - METHOD 5 (BOWTIE2/RSEM)\n")
...
for (gene_group in GENE_GROUPS) {
  ...
  gene_group_output_dir <- file.path(CV_HEATMAP_OUT_DIR, gene_group)
    ...
    for (count_type in COUNT_TYPES) {
      for (gene_type in GENE_TYPES) {
        for (label_type in LABEL_TYPES) {
          ...
          # Read raw data matrix (needed for CV calculation)
          ...
          # Generate CV heatmaps for each normalization scheme
          for (norm_scheme in NORM_SCHEMES) {
            ...
            for (sorting in sorting_options) {
              ...
              version_dir <- file.path(gene_group_output_dir, processing_level, count_type, gene_type,
                                      norm_scheme, "Original_RowGene_ColumnOrgan",
                                      sorting$sort_name)
            ...
            # Pass both normalized (for display) and raw (for CV) matrices
...
cat("\n", paste(rep("=", 60), collapse = ""), "\n")
cat("SUMMARY:", successful_heatmaps, "/", total_heatmaps, "CV heatmaps generated")
