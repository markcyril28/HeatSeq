...
source "B_M4_modules/0_configurations.sh"
source "B_M4_modules/1_logging.sh"
...
if [ "$RUN_MATRIX_CREATION" = true ]; then
    ...
    Rscript --no-save B_M4_modules/a_tximport_salmon_to_matrices_m4.R
...
if [ "$RUN_BASIC_HEATMAP" = true ]; then
    ...
    GENE_GROUPS_FILE="B_M4_modules/.gene_groups_temp.txt"
    printf "%s\n" "${GENE_GROUPS[@]}" > "$GENE_GROUPS_FILE"
    
    OVERWRITE_FILE="B_M4_modules/.overwrite_temp.txt"
    ...
    Rscript --no-save B_M4_modules/3_make_heatmap_of_matrices.R
...
if [ "$RUN_CV_HEATMAP" = true ]; then
    ...
    Rscript --no-save B_M4_modules/4_make_heatmap_with_CV_of_matrices.R
...
if [ "$RUN_BAR_GRAPHS" = true ]; then
    ...
    Rscript --no-save B_M4_modules/5_make_BarGraph_of_matrices.R
