...
# ===============================================
# TXIMPORT: SALMON TO DESEQ2 MATRICES - METHOD 4
# ===============================================
...
for (level_name in names(processing_levels)) {
  ...
  if (level_config$tx_out == FALSE) {
    ...
    if (!file.exists(tx2gene_file)) {
      ...
      cat("This file is required for gene-level summarization.\n")
  ...
  if (has_replicates) {
    ...
    # tximport provides abundance in TPM units for Salmon
...
for (level_name in names(processing_levels)) {
  level_config <- processing_levels[[level_name]]
  cat("  •", level_config$label, "- quant.sf files\n")
}
