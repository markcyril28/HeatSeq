...
process_gene_group() {
    ...
    if [ "$RUN_BASIC_HEATMAP" = true ]; then
        ...
        Rscript --vanilla b_modules_for_Method_1/b_make_heatmap_of_matrices.R \
            -d "$SCRIPT_DIR" \
            -f "$FASTA_TAG" \
            -c "counts" \
            -g "$gene_group_file"
    ...
    if [ "$RUN_CV_HEATMAP" = true ]; then
        ...
        Rscript --vanilla b_modules_for_Method_1/c_make_heatmap_with_CV_of_matrices.R \
            -d "$SCRIPT_DIR" \
            -f "$FASTA_TAG" \
            -c "counts" \
            -g "$gene_group_file"
    ...
    if [ "$RUN_BAR_GRAPHS" = true ]; then
        ...
        Rscript --vanilla b_modules_for_Method_1/d_make_BarGraph_of_matrices.R \
            -d "$SCRIPT_DIR" \
            -f "$FASTA_TAG" \
            -c "counts" \
            -g "$gene_group_file"
